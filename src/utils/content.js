...
export function normalizeContent(data) {
  ...
  if (typeof data === 'object') {
    ...
    if (data.message?.content !== undefined) return normalizeContent(data.message.content)
    if (Array.isArray(data.choices) && data.choices[0]?.message?.content) {
      return normalizeContent(data.choices[0].message.content)
    }
    if (data.delta?.content !== undefined) return normalizeContent(data.delta.content)
    if (data.delta?.text !== undefined) return normalizeContent(data.delta.text)
