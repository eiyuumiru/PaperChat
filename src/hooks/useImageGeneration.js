...
export function useImageGeneration() {
  ...
  const extractImageUrl = (response) => {
    ...
    if (typeof response === 'string' && (response.startsWith('http') || response.startsWith('data:'))) return response
  ...
  const generateImage = useCallback(async (prompt, model) => {
    ...
    try {
      ...
      let errorCode = '', errorStatus = 0, errorMessage = ''
      for (let key in err) {
      ...
      if (errorCode === 'insufficient_funds' || errorStatus === 402) {
        displayMsg = 'Hết credits API. Vui lòng nạp thêm tại puter.com để tiếp tục sử dụng.'
          ...
          const analysisResponse = await window.puter.ai.chat([
            {
              ...
              content: [
                ...
                { type: 'text', text: `Analyze this image and create a detailed prompt for regenerating it with these modifications: "${prompt}". Output ONLY the prompt.` }
              ]
            }
          ], { model: 'gpt-5-nano' })
